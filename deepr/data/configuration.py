...
class DataConfiguration:
    def __init__(self, data_configuration: Dict):
        ...
        self.orog_configuration = data_configuration.get("orog_configuration", None)
        self.lsm_configuration = data_configuration.get("lsm_configuration", None)
        self.split_coverages = data_configuration.get("split_coverages", None)
        self.data_configuration = data_configuration

    def __getitem__(self, item):
        return self.data_configuration.get(item)
    ...
    def get_features(
        self,
    ) -> "tuple[DataFileCollection | None, DataFileCollection | None, DataFileCollection | None]":
        """
            ...
            for features_date in features_dates:
                ...
                for variable in self.features_configuration["variables"]:
                    features_file = DataFile(
                        base_dir=self.features_configuration["data_location"] + features_date[:4] + "/",
                        variable=variable,
                    ...
                    if features_file.exist():
                        ...
                        print(str(features_date) + " doesn't exist in features files.")
    ...
    def get_labels(
        ...
        for split_name in DATA_SPLIT_NAMES:
            ...
            for label_date in label_dates:
                label_file = DataFile(
                    base_dir=self.label_configuration["data_location"] + label_date[:4] + "/",
                ...
                if label_file.exist():
                    ...
                    print(str(label_date) + " doesn't exist in label files.")
    ...
    def get_orog(self,
        ...
        """
        Get the list of orography files based on the orog_configuration.
        
        Returns
        -------
        list
            List of DataFile objects representing the orography files.
        Raises
        -------
        FileNotFoundError
            If no file was found in the defined orog_configuration.
        """
        if self.orog_configuration is None:
            return None, None, None
        ...
        for split_name in DATA_SPLIT_NAMES:
            ...
            # Get the dates for the orography
            orog_dates = self.get_dates(self.split_coverages[split_name])

            # Initialize the list of orogs
            orog_files = DataFileCollection(collection=[])

            # Loop through each date in the orog_configuration
            for orog_date in orog_dates:
                orog_file = DataFile(
                    base_dir=self.orog_configuration["data_location"] + orog_date[:4] + "/",
                    variable=self.orog_configuration["variable"],
                    dataset=self.orog_configuration["data_name"],
                    temporal_coverage=orog_date,
                    spatial_resolution=self.orog_configuration["spatial_resolution"],
                    spatial_coverage=self.orog_configuration["spatial_coverage"],
                )
                if orog_file.exist():
                    orog_files.append_data(orog_file)

            if not len(orog_files):
                raise FileNotFoundError(
                    "No file was found for the defined orog_configuration."
                )

            data_splits[split_name] = orog_files
    ...
    def get_lsm(self,
        ...
        """
        Get the list of land-sea mask files based on the lsm_configuration.
        
        Returns
        -------
        list
            List of DataFile objects representing the land-sea mask files.
        Raises
        -------
        FileNotFoundError
            If no file was found in the defined lsm_configuration.
        """
        if self.lsm_configuration is None:
            return None, None, None
        ...
        for split_name in DATA_SPLIT_NAMES:
            ...
            for orog_date in orog_dates:
                orog_file = DataFile(
                    base_dir=self.lsm_configuration["data_location"] + orog_date[:4] + "/",
                    variable=self.lsm_configuration["variable"],
                    dataset=self.lsm_configuration["data_name"],
                    temporal_coverage=orog_date,
                    spatial_resolution=self.lsm_configuration["spatial_resolution"],
                    spatial_coverage=self.lsm_configuration["spatial_coverage"],
                )
                if orog_file.exist():
                    orog_files.append_data(orog_file)
            ...
            if not len(orog_files):
                raise FileNotFoundError(
                    "No file was found for the defined lsm_configuration."
                )
    ...
    def get_plain_dict(self) -> dict:
        ...
        for s, period in self.split_coverages.items():
            start_date = str(period["start"]).replace("-", "/")
            end_date = str(period["end"]).replace("-", "/")
