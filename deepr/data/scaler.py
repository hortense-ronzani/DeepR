...
class XarrayStandardScaler:
    def __init__(
        ...
        if os.path.exists(self.cache_file):
            ...
            # for key in list(self.average.keys()):
            #     print(key)
            #     print(self.average[key].values)
            #     print(self.standard_deviation[key].values)
            self.average.load()
    ...
    def to_dict(self) -> str:
        ...
        elif average.ndim in [1, 2, 3]:
            average = average.tolist()
        else:
            raise NotImplementedError("Average of scaler must be 1D or 2D or 3D.")

        if len(std) == 1:
            std = float(std)
        elif std.ndim in [1, 2, 3]:
            std = std.tolist()
        else:
            raise NotImplementedError("Standard deviation of scaler must be 1D or 2D or 3D.")
