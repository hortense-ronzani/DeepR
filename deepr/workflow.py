...
class MainPipeline:
    ...
    def set_scalers(self, train_features, train_label, train_orog=None, train_lsm=None):
        # Features
        ...
        # Orography
        scaling_cfg = self.data_config.orog_configuration.get("standardization", {})
        if train_orog is not None and scaling_cfg.get("to_do", False):
            print("Fallait pas rentrer là")
            scaler_filename = Path(scaling_cfg["cache_folder"])
            os.makedirs(scaler_filename, exist_ok=True)
            scaler_filename = (
                scaler_filename / f"{scaling_cfg['method']}_orog_scale.pkl"
            )
            print("Orography")
            self.orog_scaler = XarrayStandardScaler(
                train_orog, scaling_cfg["method"], scaler_filename
            )
        else:
            logger.info("No orography standardization.")
            self.orog_scaler = None

        # Land-sea mask
        scaling_cfg = self.data_config.lsm_configuration.get("standardization", {})
        if train_lsm is not None and scaling_cfg.get("to_do", False):
            scaler_filename = Path(scaling_cfg["cache_folder"])
            os.makedirs(scaler_filename, exist_ok=True)
            scaler_filename = (
                scaler_filename / f"{scaling_cfg['method']}_lsm_scale.pkl"
            )
            print("Land-sea mask")
            self.lsm_scaler = XarrayStandardScaler(
                train_lsm, scaling_cfg["method"], scaler_filename
            )
        else:
            logger.info("No land-sea mask standardization.")
            self.lsm_scaler = None

        if self.train_config is not None and self.train_config.output_dir is not None and self.features_scaler is not None:
            ...
            with open(output_dir / "training_scale.json", "w") as f:
                f.write(json.dumps(scales, indent=4))
    ...
    def get_dataset(self) -> Tuple[DataGenerator, DataGenerator, DataGenerator]:
        ...
        # Get orography
        if self.data_config.features_configuration.get('add_auxiliary', True).get('orog-high', True):
            logger.info(f"Getting orography.")
            train_orog, val_orog, test_orog = self.data_config.get_orog()
        else:
            logger.info(f"No orography.")
            train_orog, val_orog, test_orog = None, None, None
        # Get land-sea mask
        if self.data_config.features_configuration.get('add_auxiliary', True).get('lsm-high', True):
            logger.info(f"Getting land-sea mask.")
            train_lsm, val_lsm, test_lsm = self.data_config.get_lsm()
        else:
            logger.info(f"No land-sea mask.")
            train_lsm, val_lsm, test_lsm = None, None, None

        # Set scaler with training set
        self.set_scalers(train_features, train_label, train_orog, train_lsm)
        ...
        if False:
            ...
            add_aux = self.data_config.features_configuration.get('add_auxiliary', True)
    ...
    def validate_model(self, model, dataset, config: dict, hf_repo_name: str = None):
        ...
        elif self.data_config["split_coverages"]["test"]["do"]:
    ...
    def generate_predictions(self):
        *_, ds_test = self.get_dataset()
        model, repo_name = self.load_trained_model()
        # Force adding of time dimension
        ds_test.add_auxiliary_features["time"] = True
        dl_test = DataLoader(
