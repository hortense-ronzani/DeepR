...
def get_figure_model_samples(
    ...
    for i in range(n_samples):
        if input_image is not None:
            axs[0, i].imshow(input_image[i, 0].numpy()[..., numpy.newaxis], **v_kwargs, origin='lower')
            axs[1, i].imshow(fine_image[i, 0].numpy()[..., numpy.newaxis], **v_kwargs, origin='lower')
        else:
            axs[0, i].imshow(fine_image[i, 0].numpy()[..., numpy.newaxis], **v_kwargs, origin='lower')

        # Loop over the number of rows in the column
        for r in range(n_realizations):
            # Plot the predictions
            im = axs[n_extras + r, i].imshow(
                prediction[i + r * n_samples, 0].numpy()[..., numpy.newaxis], **v_kwargs, origin='lower'
            )
