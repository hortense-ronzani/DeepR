...
def main(input_directory, output_directory, lon_min, lon_max, lat_min, lat_max):
    ...
    input_files = glob.glob(f"{input_directory}/*.nc4c")
    input_files.sort()
    ...
    for num, input_file in enumerate(input_files):
        ...
        with xarray.open_dataset(input_file) as input_data:
            # input_data = xarray.open_dataset(input_file)

            # Adjust latitude values if needed based on the dataset
            lat_min_fixed, lat_max_fixed = adjust_latitudes(
                lat_min, lat_max, input_data.lat.values
            )

            input_data_sel = input_data.where(
                input_data.lat==slice(lat_min_fixed, lat_max_fixed),
                input_data.lon==slice(lon_min, lon_max),
            )
